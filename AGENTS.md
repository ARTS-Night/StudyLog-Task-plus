...
- `src/sync/google-auth.js` は Google API 共通の OAuth・トークン処理を提供します。ログイン・ログアウト（Google側のトークン失効込み）・再認可・アカウントメール取得と、401時にキャッシュを破棄して1回だけ再試行する共通fetchを持ちます。`login()` は要求前に必ず内部で `logout()` 相当（キャッシュ破棄＋Google側の同意取り消し）を行ってから対話認証します。以前の同意やキャッシュ済みトークンが残っていると、Chromeにログイン中のアカウントで無言のまま完了してしまうため、ログイン操作のたびにアカウント選択・同意画面を出し直すための実装です（`chrome.identity.getAuthToken()` はChromeブラウザ自体のログインアカウントに依存するため、この方法でも「拡張機能内で任意のGoogleアカウントを常に選べる」ことまでは保証できません。それには別途 `launchWebAuthFlow` 用のWebアプリケーション型クライアントIDが必要です）。`src/sync/drive/drive-sync.js` は Drive 固有のフォルダ・ファイル処理だけを担当し、マイドライブの可視フォルダ `stalog_task_plus` 内の `stalog-tasks.json` を読み書きします。`manifest.json` の同じOAuthクライアントを Drive と Google Tasks で共有し、学校のGoogle Workspaceアカウントでのみログインできます。
