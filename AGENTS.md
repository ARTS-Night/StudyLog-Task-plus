...
- Chrome 上の表示名は「スタログ授業メモ」、`manifest.json` の現在のバージョンは `0.2.0` です。
...
- 対象は Chrome デスクトップと `https://portal.iwasaki.ac.jp/` 配下です。保存先は `sync` / `local` / `drive` の3モードで、`drive` の実データも物理的には `chrome.storage.local` です。物理領域の判定は必ず `TaskLifecycle.physicalStorageMode()` を使い、未設定・不正値は `sync` に倒してください（既存ユーザーの同期データを見失わないための重要な既定値）。設定ページのUI上は `sync` を非推奨（ブラウザ自体の同期機能に依存し設定が必要な場合があるため）とし、複数端末での共有には `drive` をおすすめとして先頭に表示しています。これはUI上の案内順・文言だけの変更であり、`physicalStorageMode()` の既定値（未設定・不正値は `sync`）は変更しないでください。
- `src/sync/google-auth.js` は Google API 共通の OAuth・トークン処理を提供します。ログイン・ログアウト（Google側のトークン失効込み）・再認可・アカウントメール取得と、401時にキャッシュを破棄して1回だけ再試行する共通fetchを持ちます。`login()` はキャッシュ済みトークンがあればそのまま使ってログイン状態を維持します（キャッシュ破棄は行いません）。別アカウントへ切り替えたい場合は `switchAccount()` を使い、これは要求前に必ず内部で `logout()` 相当（キャッシュ破棄＋Google側の同意取り消し）を行ってから対話認証するため、アカウント選択・同意画面を出し直します（`chrome.identity.getAuthToken()` はChromeブラウザ自体のログインアカウントに依存するため、この方法でも「拡張機能内で任意のGoogleアカウントを常に選べる」ことまでは保証できません。それには別途 `launchWebAuthFlow` 用のWebアプリケーション型クライアントIDが必要です）。設定ページには「Googleでログイン」（`login()`）と、ログイン中のみ表示される「別のアカウントでログイン」（`switchAccount()`）・「ログアウト」（`logout()`）の3つのボタンがあります。`src/sync/drive/drive-sync.js` は Drive 固有のフォルダ・ファイル処理だけを担当し、マイドライブの可視フォルダ `stalog_task_plus` 内の `stalog-tasks.json` を読み書きします。`manifest.json` の同じOAuthクライアントを Drive と Google Tasks で共有し、学校のGoogle Workspaceアカウントでのみログインできます。
- Google Tasks連携は、保存先モードと独立した任意の一方向プッシュです。スタログ側の追加・本文変更・完了切替・期限変更・削除だけを送り、Google Tasks側の編集は読み戻しません。変更検知（`processClassChange`）は `text`/`done`/`dueAt` の3項目をすべて比較し、期限だけの変更（本文・完了状態は同じ）でも送信対象にします。期限（`dueAt`）はGoogle Tasks APIの `due` フィールド（RFC3339）へ、ローカルの年月日をそのまま `YYYY-MM-DDT00:00:00.000Z` として送ります（`GoogleTasksSync` 内の `toGoogleDue()`）。Google Tasks側は `due` の時刻部分を無視して日付だけを使うため、`dueHasTime`（時刻の有無）はこちらへ送信しません。`toISOString()` 経由でUTCへ変換すると、タイムゾーンによって日付が前後にずれるため使わないでください。新規作成時は期限が無ければ `due` を省略し、更新時は期限の有無に関わらず必ず `due`（値または `null`）を送って、削除も反映できるようにします。`src/sync/google-tasks/google-tasks-sync.js` は状態を持たないAPIラッパー、`src/sync/google-tasks/google-tasks-mirror-background.js` は授業IDごとのタスクリスト対応表、タスクIDの書き戻し、タスク単位の永続outboxを担当します。操作意図はAPI呼出し前に `__google_tasks_pending_ops__` へ保存し、失敗分をService Worker起動時と2分間隔のアラームで個別に再試行します。同じ授業変更に含まれる複数タスクのAPI処理は並列に行いますが、outboxとタスクリスト対応表のread-modify-writeはそれぞれ直列化し、同一授業のタスクリスト解決中Promiseを共有して重複作成を防ぎます。リモート作成後は返されたリストID・タスクIDをoutboxへ先に保存してからローカルへ書き戻し、書き戻し失敗時の重複作成を防ぎます。更新先リストの404時は古いタスクIDを再利用せず、再解決したリストへ最新のローカル内容で新規作成して両IDを書き戻します。削除の404は既に削除済みとして完了します。授業ごとに科目名と同名の平坦なタスクリストを1つ使います。連携を有効にした時点の既存タスクは自動送信対象にならないため、設定ページの「既存タスクを今すぐ同期」ボタン（`chrome.runtime.sendMessage({type:"google-tasks-backfill"})`）でまだ`googleTaskId`の無いタスクだけをまとめて送信キューへ積めます。書き戻し先のタスクが既にローカルから消えている場合（同期無効中の削除など）は、作成済みのリモートタスクを片付けたうえで操作自体を破棄し、無限リトライにしません。同一授業内の並列送信では、outboxが完全に空になるまでエラー表示を消しません（1件の成功が別の1件の失敗を覆い隠さないため）。
...
| `tests/google-tasks-sync.test.js` | Google Tasksのリスト検索・作成、タスク追加・更新・削除のAPI契約、期限（due）の日付変換・省略・削除（`due: null`）を確認するテスト |
| `tests/google-tasks-mirror.test.js` | Google Tasksへの差分送信、永続outboxとアラーム再試行、同一授業の並列追加でタスクリストを重複作成しないこと、競合安全なID書き戻し、404自己修復、同期無効中に削除されたタスクのゴースト整理、並列送信時の部分失敗でエラー表示を誤って消さないこと、本文・完了状態が同じで期限だけ変更・削除した場合も送信対象になることを確認するテスト |
...
| キー | 内容 |
| --- | --- |
| `__storage_mode__` | `sync` / `local` / `drive`。未設定・不正値は `sync`（既存ユーザー向けの既定値、`physicalStorageMode()`は変更しない）。明示的な `local` / `drive` だけが `chrome.storage.local`（判定は `TaskLifecycle.physicalStorageMode()`）。新規インストール時だけ `mutation-lock-background.js` の `chrome.runtime.onInstalled`（`reason === "install"`）で明示的に `"local"` を書き込み、新規ユーザーの初期選択をローカル保存にしている |
...
- `google-tasks-sync.test.js` はタスクリストの作成/解決、タスクの作成・更新（未完了化時の`completed: null`送信を含む）・削除、404削除の成功扱い、期限（`due`）を日付部分だけのRFC3339へ変換して送ること・期限なしの新規作成では`due`を省略すること・編集での期限削除は`due: null`を送ることを偽`fetch`で確認します。
- `google-tasks-mirror.test.js` は永続化された保留オペレーション（outbox）の再試行、授業ごとのタスクリスト解決の並行安全性（同時解決の一本化、解決中マップの安全な差し替え）、成功時のみのクリア、同一授業内タスクの並列プッシュ、同期無効中に削除されたタスクのゴースト整理（無限リトライにならないこと）、並列送信中の部分失敗でエラー表示を誤って消さないこと、本文・完了状態が変わらず期限だけを変更・削除した場合も差分として検知し送信することを確認します。
...
8. 設定画面でGoogle Tasks連携を有効化すると、授業ごとにGoogle Tasksへタスクリストが作成され、追加・編集・完了・期限変更・削除がGoogle Tasks側へ反映され、最終同期時刻が表示される（Google Tasks側の変更はStalogへ反映されない一方向）。期限を設定したタスクは、Google Tasks側でも該当日付に期限が表示される（時刻はGoogle Tasksの仕様上表示されない）。
...
- 追加・完了日時、本文・授業名・状態の絞り込み、任意の期限（日付/日付＋時刻）、追加日順・締切が近い順のワンタップ並び替えは実装済みです。タグ、授業に紐づかないカレンダー予定、独自バックエンドは未実装です。追加する場合は既存データの後方互換とUIの簡潔さを維持してください。
