...
- 専用タスク画面では、マイページから取得した年度・授業名を選んでタスクを追加し、すべてのタスクを編集・削除・完了切替できます。「追加先の授業検索」と「保存済みタスク検索」を分離し、タスク本文・授業名の複数語検索と、未完了/完了の状態絞り込みができます。
- 専用画面の「完了タスクの自動削除」で、完了から1/3/7/14/30/90日後の整理を選べます。既定は自動削除しない設定で、日数設定は端末ごとに保存します。期限を過ぎたタスクは、その端末で次にスタログか拡張画面を開いたときに削除され、同期保存のタスク自体は他の端末からも削除されます。完了日時のない旧タスクは対象外です。
...
- 専用画面のタスク検索・状態絞り込み・クリア、自動削除日数の保存が動作する。
